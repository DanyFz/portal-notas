  ...
  return (
    <main className="flex-1 min-h-screen bg-[#151d18] text-[#EDE5D8] academic-paper-bg">
      ...
      <header className="border-b border-[rgba(217,203,182,0.12)] bg-[#151d18]/95 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-3.5 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <span className="font-serif font-bold text-xs sm:text-sm text-[#FAF6EE] block leading-tight">Expediente Académico</span>
              <span className="text-[10px] sm:text-[11px] text-[#A89F8D] hidden xs:block">Calificaciones y asistencia</span>
            </div>
          </div>
          <button onClick={handleLogout} className="text-xs text-[#A89F8D] hover:text-[#FAF6EE] flex items-center gap-1.5 transition-colors cursor-pointer group py-1">
      ...
      <div className="max-w-5xl mx-auto px-3.5 sm:px-6 py-6 sm:py-8 space-y-5 sm:space-y-6">
        {/* Student header info */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#1b2620] border border-[rgba(217,203,182,0.14)] rounded-2xl p-4 sm:p-6 shadow-sm">
          <div className="flex items-start sm:items-center gap-3.5 sm:gap-4 min-w-0">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-[#223028] border border-[rgba(217,203,182,0.18)] flex items-center justify-center text-[#C8B99D] text-lg sm:text-xl font-serif font-bold shrink-0">
          ...
          <div className="w-full sm:w-auto">
            <button
              ...
              className="academic-btn-primary w-full sm:w-auto px-4 py-2.5 text-xs font-semibold rounded-lg flex items-center justify-center gap-2 cursor-pointer shadow-sm hover:brightness-110 active:scale-[0.98] transition-all"
              title="Registrar Asistencia a una sesión / quiz"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-[#141c17]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3.5">
          {/* Average score card */}
          <div className={`academic-card rounded-xl p-3.5 sm:p-4 col-span-2 sm:col-span-2 md:col-span-1 ${isPassing ? 'border-[#7A8F73]/50' : 'border-[#d9534f]/50'}`}>
            <p className="text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider">Promedio Acumulado</p>
            <p className={`text-2xl sm:text-3xl font-extrabold mt-0.5 sm:mt-1 ${isPassing ? 'text-[#FAF6EE]' : 'text-[#d9534f]'}`}>
              {avg !== null ? avg.toFixed(2) : "N/A"}
            </p>
            <p className="text-[11px] sm:text-xs mt-1 font-medium" style={{color: isPassing ? '#7A8F73' : '#d9534f'}}>
              {isPassing ? "✓ Estado: Aprobando" : "✗ Por debajo de 3.0"}
            </p>
          </div>
          
          {/* Attendance stats */}
          <div className="academic-card rounded-xl p-3.5 sm:p-4">
            <p className="text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider">Asistencias</p>
            <p className="text-2xl sm:text-3xl font-extrabold mt-0.5 sm:mt-1 text-[#7A8F73]">{presentCount}</p>
            <p className="text-[11px] sm:text-xs text-[#A89F8D] mt-0.5">de {totalAttendance} sesiones</p>
          </div>
          
          <div className="academic-card rounded-xl p-3.5 sm:p-4">
            <p className="text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider">Ausencias</p>
            <p className="text-2xl sm:text-3xl font-extrabold mt-0.5 sm:mt-1 text-[#d9534f]">{absentCount}</p>
            <p className="text-[11px] sm:text-xs text-[#d9534f]/70 mt-0.5">registradas</p>
          </div>
          
          <div className="academic-card rounded-xl p-3.5 sm:p-4">
            <p className="text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider">Excusas</p>
            <p className="text-2xl sm:text-3xl font-extrabold mt-0.5 sm:mt-1 text-[#C8B99D]">{excusaCount}</p>
            <p className="text-[11px] sm:text-xs text-[#A89F8D] mt-0.5">justificadas</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="academic-card rounded-xl overflow-hidden">
            <div className="p-3.5 sm:p-4 border-b border-[rgba(217,203,182,0.12)] bg-[#202d26] flex items-center gap-2">
            ...
            <div className="p-3 sm:p-4 overflow-x-auto">
              {gradeEntries.length === 0 ? (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr>
                      <th className="text-left py-2 px-2.5 sm:px-3 text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider border-b border-[rgba(217,203,182,0.12)]">Evaluación</th>
                      <th className="text-right py-2 px-2.5 sm:px-3 text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider border-b border-[rgba(217,203,182,0.12)]">Nota</th>
                    </tr>
                  </thead>
                  <tbody>
                    {gradeEntries.map(([quiz, grade]) => {
                      ...
                      return (
                        <tr key={quiz} className="hover:bg-[#223028] transition-colors">
                          <td className="py-2.5 px-2.5 sm:px-3 font-medium text-[#EDE5D8]">{quiz}</td>
                          <td className="py-2.5 px-2.5 sm:px-3 text-right">
                            {grade === null ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#C8B99D]/15 text-[#C8B99D] border border-[#C8B99D]/25 whitespace-nowrap">
                                Pendiente
                              </span>
                            ) : grade === 0 && !isPresent ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#d9534f]/15 text-[#d9534f] border border-[#d9534f]/25 whitespace-nowrap">
                                0.0 — Ausente
                              </span>
                            ) : (
                              <span className={`font-bold font-mono text-sm sm:text-xs ${grade >= 3.0 ? 'text-[#7A8F73]' : 'text-[#d9534f]'}`}>
                                {grade.toFixed(1)}
                              </span>
                            )}
                          </td>
          ...
          {/* Attendance Table */}
          <div className="academic-card rounded-xl overflow-hidden">
            <div className="p-3.5 sm:p-4 border-b border-[rgba(217,203,182,0.12)] bg-[#202d26] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <button
                type="button"
                onClick={() => openRegisterModal()}
                className="self-start sm:self-auto px-2.5 py-1 text-[11px] font-medium rounded-md bg-[#223028] border border-[rgba(217,203,182,0.15)] text-[#C8B99D] hover:border-[#7A8F73] hover:text-[#FAF6EE] transition-all cursor-pointer flex items-center gap-1.5"
              >
              ...
              {attendanceEntries.length === 0 ? (
                ...
                <div className="max-h-80 overflow-y-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr>
                        <th className="text-left py-2 px-2.5 sm:px-3 text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider border-b border-[rgba(217,203,182,0.12)]">Evaluación / Sesión</th>
                        <th className="text-right py-2 px-2.5 sm:px-3 text-[10px] sm:text-[11px] font-semibold text-[#A89F8D] uppercase tracking-wider border-b border-[rgba(217,203,182,0.12)]">Estado</th>
                      </tr>
                    </thead>
                    <tbody>
                      {attendanceEntries.map(([date, status]) => (
                        <tr key={date} className="hover:bg-[#223028] transition-colors">
                          <td className="py-2.5 px-2.5 sm:px-3 font-medium text-[#EDE5D8]">{date}</td>
                          <td className="py-2.5 px-2.5 sm:px-3 text-right">
                            {status === "presente" ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#7A8F73]/15 text-[#7A8F73] border border-[#7A8F73]/30 whitespace-nowrap">
                                ✓ Asistencia
                              </span>
                            ) : status === "excusa" ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#C8B99D]/15 text-[#C8B99D] border border-[#C8B99D]/25 whitespace-nowrap">
                              ...
                              <span
                                className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#d9534f]/15 text-[#d9534f] border border-[#d9534f]/25 whitespace-nowrap"
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-[#223028] text-[#C8B99D] border border-[rgba(217,203,182,0.2)] hover:border-[#7A8F73] hover:text-[#FAF6EE] transition-all cursor-pointer whitespace-nowrap"
                                title="Sesión abierta. Haz clic para registrar tu asistencia"
                              >
                                <span>⏳ Pendiente</span>
                                <span className="text-[10px] text-[#7A8F73] underline ml-0.5">Registrar</span>
                              </button>
                            )}
                          </td>
                        </tr>
